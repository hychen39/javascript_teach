...
Given the following HTML structure, we register the event listeners to HTMLElement objects that do not have the class `noListener`.
...
1. Target Phase: P (Target)
2. Bubbling Phase: P > DIV > FORM > DIV > BODY > HTML (Target to Top)

See the complete example in [ex_11_4.html](ex_11_04.html)

![bg right:50% fit](img/24-Dec-07-21-30-56.png)
...
- Benefit: Save the lines of code and improve the performance of the web page.
...
See [Lab 11-2](lab_11_02.md) for a step-by-step guide to complete the lab.
...
![w:400px](img/24-09-01-09-31-44.png)

- After clicking the email field, the field's background color changes to yellow.
- After leaving the email field to the Phone field, the email field's background color changes to white and the Phone field's background color changes to yellow.

- use the Event Delegation pattern 
  - where a single event listener is registered to the parent element `form`.

Both the `focus` and `blur` events invoke the handler function in the **capturing phase** 
  - so that the field's background color changes before the cursor moves to the next field.
...
- Press left-shift and b key, returns "ShiftLeft" and "KeyB"
...
function isNumberKey(event) {
    ...
    if (isNaN(event.key) && !exceptionKeys.includes(event.code) === true 
        || event.code =='Space'){
        // not a number key and (not an exception key) or (a space key)
        // prevent the default behavior of the keydown event, 
        //but not stop the event propagation
