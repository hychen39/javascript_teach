...
- Use `let` to declare local variables in a block or function (block scope).
- Use `const` to declare constants.
- Use `var` to declare global variables in a script (global scope). 
...
console.log(i);  // out of scope; ReferenceError: i is not defined
```
