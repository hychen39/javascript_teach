...
function hoisting() {
  ...
  // to the top of the function (hoisted) without initialization, the value is `undefined`.
...
Note: 
- JS engine will not hoist the variables declared with `let` and `const`.
...
- The inner function is within the scope of the outer function. So, it can access the variables and parameters of the outer function.
...
- Rule 3:  We cannot invoke the inner function from the outside of the outer function.
...
- This happens commonly when you return a (nested) function from a (outer) function within which the nested function was defined.

<img src="img/24-Oct-18-12-35-14.png" style="width:60%;" />
...
funcN1 = f(20);
// funcN1 has the lexical variable b = 20
const k = funcN1();
console.log(k); // 40

funcN2 = f(30);
// funcN2 has the lexical variable b = 30
const m = funcN2()
console.log(m); // 50
```
Key Point: the parameter `b` becomes a private variable of the inner function.
...
This lab demonstrates using the closure technique to create a private variable for each event listener.
...
- When user clicks the button, the corresponding function is invoked. 
- However, the parameters `idx` and `counter` are no longer available, because they don't have the lexical scope to the inner function.

![bg right 100%](img/24-07-31-13-28-13.png)
...
- IIFE is an expression that combine the function definition and invocation.
  - use it when 
    ...
    - to create private properties for a function/object.
...
- Variable hoisting for `var` variables in a function.
- Nested functions and closures.
  - A closure is a function that binds to the variables of the outer function when it executes.
  - The closure is a common pattern and a powerful technique in JavaScript.
